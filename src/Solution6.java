...
/**
 * Algorithm notes:
 * Solution 5 but level up and mentoring is included
 */
public class Solution6 implements Solution {
                        ...
                        .anyMatch(contributorSkill ->
                                contributorSkill.getName().equals(projectSkill.getName()) &&
                                        (contributorSkill.getLevel() >= projectSkill.getRequiredLevel() ||
                                                canBeMentored(usedContributors, contributorSkill, projectSkill)
                                        ))
                )
    ...
    boolean canBeMentored(List<Contributor> usedContributors, Contributor.Skill contributorSkill, Project.Skill projectSkill) {
        return (contributorSkill.getLevel() == projectSkill.getRequiredLevel() - 1) &&
                usedContributors.stream().anyMatch(contributor -> {
                    Optional<Contributor.Skill> possSkill = contributor.getSkillList().stream()
                            .filter(skill -> skill.getName().equals(projectSkill.getName()))
                            .findFirst();
                    if (possSkill.isPresent()) {
                        return possSkill.get().getLevel() >= projectSkill.getRequiredLevel();
                    }
                    return false;
                });
    }
